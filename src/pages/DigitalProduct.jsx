...
const DigitalProductsPage = () => {
  ...
  useEffect(() => {
    const fetchUserAndProducts = async () => {
      try {
        ...
        const productsResponse = await axios.get('http://192.168.43.102:8000/api/digital-products', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDeleteProduct = async (product) => {
    ...
    try {
      ...
      await axios.delete(`http://192.168.43.102:8000/api/digital-products/${product.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const getImageUrl = (imagePath) => {
    ...
    return `http://192.168.43.102:8000/storage/${imagePath}`;
  };
