...
const AdminAddStore = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.43.102:8000/api/store', formData, {
        headers: {
      ...
      if (error.response?.status === 401) {
        try {
          const refreshResponse = await axios.post('http://192.168.43.102:8000/api/auth/refresh', {}, {
            headers: {
          ...
          const retryResponse = await axios.post('http://192.168.43.102:8000/api/store', formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
